...
The script reads ``./<ticker>/<start-date>-<end-date>-<filter>/<tickers>-<range>.csv``
generated by :mod:`eldoradoBacktest`. The ``<ticker>`` directory comes from the original
tickers argument before portfolio expansion with spaces and special characters
removed. It filters rows between the specified
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "tickers",
        help="Ticker symbols used for the analysis",
    )
...
def main() -> None:
    ...
    ticker_label = sanitize_ticker_string(args.tickers)
    root = Path(ticker_label) / f"{args.start}-{args.end}-{args.filter.replace(' ', '_')}"
