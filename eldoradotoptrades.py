...
The script reads ``./tickers/<start-date>-<end-date>-<filter>/<ticker-list>-<range>.csv``
generated by :mod:`eldoradoBacktest`. The ``<ticker-list>`` portion comes from
the raw ticker arguments passed to ``--ticker-list`` before portfolio expansion
with spaces and special characters removed. It filters rows between the
specified ``--start`` and ``--end`` dates, matching the ``--range`` value when
an ``open_range_minutes`` or ``range`` column is present.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Tickers or portfolio names used to locate the data file",
    )
...
def main() -> None:
    ...
    ticker_label = sanitize_ticker_string(args.ticker_list)
    root = Path("tickers") / f"{args.start}-{args.end}-{args.filter.replace(' ', '_')}"
    data_file = root / f"{ticker_label}-{args.range}.csv"
